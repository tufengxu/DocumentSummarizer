...
class Summarizer:
    ...
    def __init__(self, stopWordsPath, text):
        self.parser = Parser(stopWordsPath,text)
        self.ideal = 18

    def summarize(self, text, title, source, category):
        #分词后的句子
        text_cutted = self.parser.text_cutted
        #分词去除停用词后合并的句子
        text_clean = self.parser.text_clean
        sentencesall = list(self.parser.splitSentences(text_cutted))
        titleWords = self.parser.removePunctations(title)
        titleWords = self.parser.splitWords(title)
        (keywords, wordCount) = self.parser.getTopCountwords(text_clean)

        topKeywords = self.getTopKeywords(keywords[:], wordCount, source, category)

        result = self.computeScore(sentencesall, titleWords, topKeywords)
    ...
    def computeScore(self, sentences, titleWords, topKeywords):
        ...
        for i, sentence in enumerate(sentences):
            ...
            sentenceLength = self.parser.getSentenceLengthScore(words, self.ideal)
            sentencePosition = self.parser.getSentencePositionScore(i, len(list(sentences)))
